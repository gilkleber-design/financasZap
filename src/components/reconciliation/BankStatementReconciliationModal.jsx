...
export default function BankStatementReconciliationModal({ open, onOpenChange }) {
  ...
  const [hideConciliated, setHideConciliated] = useState(false);
  ...
  const visibleRows = useMemo(() => hideConciliated ? rows.filter(r => r.status === 'orphan') : rows, [rows, hideConciliated]);
      ...
      <DialogContent className="flex flex-col max-h-[90vh] max-w-7xl p-0 font-sora">
        ...
        <div className="flex gap-2 p-4">
            ...
            <Button variant="outline" onClick={() => setHideConciliated(!hideConciliated)}>
                {hideConciliated ? <Eye className="w-4 h-4 mr-2" /> : <EyeOff className="w-4 h-4 mr-2" />}
                                        ...
                                        <PopoverContent className="w-[400px] p-0">
                                            <Command>
                                                ...
                                                <CommandList className="max-h-[300px] overflow-y-auto">
                                                    <CommandGroup>
                                                        {candidates.filter(c => {
                                                            const isTransfer = c.kind === 'transaction' && c.type === 'transfer';
                                                            if (row.type === 'income') return c.kind === 'receivable' || isTransfer || (c.kind === 'transaction' && c.type === 'income');
                                                            return c.kind === 'payable' || isTransfer || (c.kind === 'transaction' && c.type === 'expense');
                                                        }).map(c => (
