...
export default function ReconciliationConfront({ realizado, contasAPagar }) {
  const diff = realizado - contasAPagar;
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4">
        <div className="text-center flex-1">
          <div className="text-xs text-slate-400 mb-1">Realizado</div>
          <div className="text-2xl font-bold text-slate-800">{fmt(realizado)}</div>
        ...
        <div className={`text-center flex-1 rounded-xl px-4 py-3 ${hasDiff ? 'bg-amber-50 border border-amber-200' : 'bg-emerald-50 border border-emerald-200'}`}>
          ...
          <div className={`text-2xl font-bold ${hasDiff ? (diff > 0 ? 'text-amber-600' : 'text-red-600') : 'text-emerald-600'}`}>
            {diff >= 0 ? '+' : ''}{fmt(diff)}
          </div>
          <div className="text-[11px] text-slate-400 mt-0.5">
            {!hasDiff ? 'Fechado ✓' : diff > 0 ? 'Gastou além do previsto' : 'Contas sem pagamento'}
          </div>
        </div>
      </div>
    </div>
